...
Remote operations apply only to canonical base checkouts. `vcm sync` fetches each configured trunk into its cached remote-tracking ref without moving local state. `vcm pull` requires every canonical repository to be clean with its configured trunk checked out, then rebases children in dependency order and the root last. Change worktrees never fetch or pull; `vcm refresh`, invoked from inside the target Change, merges advanced canonical local trunks into those worktrees.
